...
**v2.0.0** · Полный рефакторинг v1.0.0 · Bash 4+

Быстрая диагностика Linux-сервера одной командой. Подходит для инженеров техподдержки: запустил по SSH — получил за 10–30 секунд сводный отчёт о состоянии машины.
...
- Убран сломанный fallback для `bc`, который тихо округлял дроби до целых — теперь для float используется `awk`.
...
**Безопасность:**
- `fp1_login` выводит пароль отдельно от URL (не утекает через `ps`/history).
- Добавлен `--data-urlencode` для curl-запросов к API панелей (vesta, directadmin).
- Флаг `--no-panel-login` позволяет детектить панели без создания временных пользователей в `.htpasswd`.
